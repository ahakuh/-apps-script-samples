...
Snippets.prototype.getValues = (spreadsheetId, range) => {
  ...
  spreadsheetId = spreadsheetId || "1fMwuBSyzyDQ7fH5LX5pM_QJYTO3YdgpeQqhXKw4A0dw"
  range = range || "users!A1:F"
  try {
 ...
 */
Snippets.prototype.updateValues = (spreadsheetId, range, valueInputOption, _values) => {
  // This code uses the Sheets Advanced Service, but for most use cases
  // the built-in method SpreadsheetApp.getActiveSpreadsheet()
  //     .getRange(range).setValues(values) is more appropriate.
  let values = [
 ...
 * @returns {*} spreadsheet with updated values
 */
Snippets.prototype.batchUpdateValues = (spreadsheetId, range, valueInputOption, _values) => {
    // This code uses the Sheets Advanced Service, but for most use cases
    // the built-in method SpreadsheetApp.getActiveSpreadsheet()
    //     .getRange(range).setValues(values) is more appropriate.
    let values = [
 ...
 */
Snippets.prototype.appendValues = (spreadsheetId, range, valueInputOption, _values) => {
  let values = [
    [
      // Cell values ...
    ]
...
const snippets = () => {
  const values = new Snippets().getValues()
  var [headers,...rows] = values.values
  headers = headers
    .map(h => ({
      "text": h.toUpperCase().trim().slice(0,1) + h.toLowerCase().slice(1,h.length).trim(),
      "value": h.toLowerCase().trim(),
    }));
  console.log(headers)  
  debugger
